...
日程签名带日期（`daily_greeting:2026-03-02:morning`）：早安每天该说一次，
...
| 落定 | `POST /v1/companion/proactive/deliveries` | 实际发送时原子写助手消息、`sent`、故事状态；保存 `deliveryId` 响应 |

落定按 `deliveryId` 幂等，与开场领取同构：同一个 id 重放拿到同一份结果，
不会把一条主动记成两条。故事签名按 arc + beat 去重，不带日期；`sent` 记录不再 3 天后删除，
否则既会丢失跨日去重，也会忘掉持续未回应。`expired` 立即清出队列。

## 对话动作与可接话钩子（本轮修复）

| conversationMove | 作用 | 常用 replyHook |
| --- | --- | --- |
| close | 用户明确结束，不再推进 | none |
| hold | 克制陪伴，不强行深入 | none |
| deepen | 围绕当前具体细节深入，最多一个问题 | specific_curiosity |
| reciprocate | 回答或贡献自身判断、偏好 | personal_reaction |
| play | 关系权限允许时接梗、轻微调侃 | playful_hook |
| bridge | 有权限、有依据才提共同记忆 | memory_callback |
| pivot | 话题耗尽后换一个具体话题 | personal_reaction / self_disclosure |

规则顺序：当前停止边界 → 关系分寸 → 直接回答用户问题 → 换话题信号 → 开心接梗 / 共同记忆 →
具体深入。最近两轮助手出现过问题时，普通分享改用个人反应，避免连续采访。
明确提问时 `answerFirst=true`，回答优先，不因用户用了问号就自动回问。
明确说“不想聊、别问、算了”必须 close/hold；禁止改成别的话题继续索要回应。

普通分享通常贡献新观察、观点、具体好奇点、轻玩笑或有据记忆，不只发回执。
开心允许共同兴奋；低落可以问一个轻问题，后面用具体反应陪伴而不继续盘问。
单次短回复承接上一轮；明确要求换话题，或连续两次“嗯、确实、对”等低信息回应，可选择 pivot。
不能把沉默本身当作授权，也不能在用户结束后 pivot。
`self_disclosure` 只使用已提供的虚拟日常/故事；记忆或生活依据缺失时退回个人观点，不能补造经历。

保留客服话术、固定邀请、强行追问的限制；可接话不等于每条以问号结尾。
规则兜底会针对散步、作品、食物、完成事件给具体反应；无法回答事实问题时坦白能力不足。
这些是有限规则，不能替代真实模型的丰富表达，也不是自然度已经验收的证明。

主动内容采用同样的原则，但不再调用模型做决策或表达：只对被闸门许可的候选进行确定性表达。
同一消息跨端显示是同步，不是重复发送；每个客户端按稳定 messageId 只展示一次。
所有主动发送只经 POST delivery 落入共享历史；手机不再调用旧开场领取接口，桌面不再显示 GET 评估值。
发送压力按持久化记录计算，不把只读评估、defer/drop 或模型失败当成“用户未回应”。
过去 20 分钟桌面免打扰快照约束其他设备；夜间约定例外只放行该 follow_up，不能顺带放行其他候选。
回复的 30 分钟 solicited 判定、独立涨分上限和故事真实事件条件不变。
